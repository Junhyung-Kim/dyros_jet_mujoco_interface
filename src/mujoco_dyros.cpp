...
//  std::cout << "sensor1 " << d->sensordata[0] << std::endl;
//  std::cout << "joint1 " << d->qpos[8]<<std::endl;
//  std::cout << "sensor2 " << d->sensordata[1] << std::endl;
//  std::cout << "joint2 " << d->qpos[10]<<std::endl;
...
/*
  ...
  for(int i=0;i<m->nu;i++)
  {
    torque_mj[i]= joint_set_msg_.position[i];
  }
  mju_copy(d->ctrl,torque_mj, m->nu); //ctrl*/
...
void jointset_callback(const sensor_msgs::JointStateConstPtr& msg)
{
  ...
  if(joint_set_msg_.position.size()==m->nu)
  {

    for(int i=0;i<m->nu;i++)
    {
      joint_set_msg_.position[i]= msg->position[i];
    }
  }
  else{
...
void loadmodel(GLFWwindow* window, const char* filename)
{
  ...
  //print model detail data
    ...
    std::cout<<"number of geom =" << m->ngeom<< std::endl;
   ...
   std::cout<<"JOINT NAMES"<<std::endl;
    ...
    std::cout <<std::endl<< "SENSOR NAMES "<<std::endl;
    for(int i=0;i<m->nsensor;i++){
      std::string buffer(m->names + m->name_sensoradr[i]);
      std::cout<<"sensor id : "<<i<<"  name : "<<buffer<<"  data size : "<<m->sensor_dim[i]<<std::endl;
    }
    std::cout <<std::endl<< "GEOM NAMES" << std::endl;
    for(int i=0; i<m->ngeom;i++){
      std::string buffer(m->names + m->name_geomadr[i]);
      std::cout<<"geom id : "<<i<<"  name : "<<buffer<<std::endl;
    }

    std::cout<<"PELVIS_height ankle height"<<std::endl;
    std::cout<<d->xpos[mj_name2id(m,mjOBJ_BODY,"base_link")*3+2]<<std::endl;
    std::cout<<d->xpos[mj_name2id(m,mjOBJ_BODY,"R_AnkleRoll_Link")*3+2]<<std::endl;
